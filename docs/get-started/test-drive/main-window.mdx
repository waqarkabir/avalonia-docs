...
MainWindow is the parent window of the app. Note that it has only a tag "<views:MainView />" which is a UserControl.

## The MainView User Control
Inside the MainView.axaml file, you will see a `<TextBlock>...</TextBlock>` XAML tag. This represents a `TextBlock` control which draws 
